---
title: "Using Snowflake as a data source to train ML models with Amazon SageMaker"
format:
  html:
...
![Architecture](img/snowflake-sagemaker-page-1.png){#fig-architecture}
...
1. The trained ML model is stored in an Amazon S3 bucket.
...
1. In your AWS console navigate to Amazon SageMaker -> Notebooks instances and then click on `Open JupyterLab` for `aws-aiml-blogpost-sagemaker-snowflake-example`.
    ...
    ![Open JupyterLab](img/sm-nb-jl.png){#fig-open-jl}
...
1. Now click on `Run All Cells` to run the code in this notebook, this will download the dataset locally into the notebook and then ingest it into the Snowflake table.
    ...
    ![Notebook Run All Cells](img/sm-nb-runall.png){#fig-notebook-run-all-cells}
...
1. Close the notebook once all cells run without any error. Your data is now available in Snowflake. The following screenshot shows the `california_housing` table created in Snowflake.
   ...
   ![Snowflake Table](img/snowflake-table.png){#fig-snowflake-table}

#### Open sagemaker-snowflake-example.ipynb notebook

![Open SageMaker Snowflake Notebook](img/sm_snowflake_example.png){#fig-open-blog-post-notebook}
   
This notebook will create a custom training container with SnowFlake connection, extract data from SnowFlake into the training instance's ephemeral storage without staging it in Amazon S3, and perform Distributed Data Parallel (DDP) XGBoost model training on the data. DDP Training is not required for model training on such a small dataset; it is included here for illustration of yet another recently released SageMaker feature.
...
1. Once the model training is completed the trained model is available as `model.tar.gz` file in the default SageMaker bucket for the region.
...
We encourage you to learn more by visiting the [Amazon SageMaker Python SDK](https://sagemaker.readthedocs.io/en/stable/) and try out building a solution using the sample implementation provided in this blog post and a dataset relevant to your business. If you have questions or suggestions, leave a comment.

  *  *  *  *  *

## Author bio

<img style="float: left;" src="img/Amit.png">&nbsp;Amit Arora is an AI and ML specialist architect at Amazon Web Services, helping enterprise customers use cloud-based machine learning services to rapidly scale their innovations. He is also an adjunct lecturer in M.S. data science and analytics program at Georgetown University in Washington D.C.

<br><br>

<img style="float: left;" src="img/Divya.jfif">&nbsp;Divya Muralidharan is a Solutions Architect at Amazon Web Services. She is passionate about helping enterprise customers solve business problems with technology. She has a Masters in Computer Science from Rochester Institute of Technology. Outside of office she spends time cooking, singing and growing plants.
